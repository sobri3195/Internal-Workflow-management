...
export default function ReviewQueue() {
  ...
  const { data, isLoading } = useQuery('reviewQueue', async () => {
    const response = await axios.get('/api/documents?status=review1,review2,review3');
    return response.data;
  });

  const reviewMutation = useMutation(
    async ({ docId, action, notes }) => {
      await axios.post(`/api/review/${docId}/review`, { action, notes });
    },
    {
      onSuccess: () => {
  ...
  const handleReview = () => {
    if (!action) {
      toast.error('Pilih aksi review');
      return;
    }
    reviewMutation.mutate({ docId: selectedDoc, action, notes });
  };
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8">
      <h1 className="text-2xl font-semibold text-gray-900 mb-6">Review Queue</h1>
      ...
      {selectedDoc && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h3 className="text-lg font-medium mb-4">Review Dokumen</h3>
